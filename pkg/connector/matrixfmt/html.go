...
type HTMLParser struct {
	GetUsernameFromMXID func(context.Context, id.UserID) string
}
...
func (parser *HTMLParser) linkToString(node *html.Node, ctx Context) *EntityString {
	...
	if err == nil && parsedMatrix != nil && parsedMatrix.Sigil1 == '@' {
		...
		username := parser.GetUsernameFromMXID(ctx.Ctx, mxid)
		if username == "" {
			return str
		}
		return NewEntityString("@" + username).Format(Mention{
			ID: username,
		})
	}
