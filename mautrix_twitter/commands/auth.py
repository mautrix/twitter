...
@command_handler(
    ...
    help_text="Log in to Twitter manually",
)
...
async def enter_login_cookies(evt: CommandEvent) -> None:
    if not evt.sender.command_status["auth_token"]:
        if len(evt.args) == 0:
            await evt.reply(
                "Please enter the value of the `auth_token` cookie, or use "
                "the `cancel` command to cancel."
            )
            return
        evt.sender.command_status["auth_token"] = evt.args[0]
        await evt.reply(
            "Now do the last step again, but find the value of the `ct0` row instead. "
